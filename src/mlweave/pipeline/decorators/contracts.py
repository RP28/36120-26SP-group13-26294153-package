...
# Bound temporary boolean allocations for large ndarray / Series scans. The
# value is intentionally internal so the public decorator API stays simple.
...
def _ndarray_contains_missing(array: np.ndarray) -> bool:
    ...
    if np.issubdtype(array.dtype, np.str_) or np.issubdtype(array.dtype, np.bytes_):
        # Fixed-width NumPy string arrays cannot represent Python None/NaN/NaT
        # as missing values; strings such as "nan" are ordinary category text.
        return False

    # Object/mixed arrays need scalar-safe checks. Process in chunks to
    # keep cache locality reasonable while retaining early exit behaviour.
    for start in range(0, flat.size, _SCAN_CHUNK_SIZE):
        chunk = flat[start : start + _SCAN_CHUNK_SIZE]
        for value in chunk:
            if value is None:
...
def _contains_missing(X: Any) -> bool:
    ...
    # sklearn transformers commonly accept/produce scipy sparse matrices. Only
    # stored values need scanning; implicit sparse zeros are ordinary values.
...
def _validate_required_columns(X: Any, columns: tuple[Any, ...]) -> None:
    ...
    # Membership against pandas Index is efficient and avoids converting every
    # column label into a Python list on each fit/transform call.
...
def _validate_preserve_columns(
    ...
    if before_names is not None and after_names is not None:
        # Fast path for the overwhelmingly common case where labels/order are
        # identical. Counter is only constructed when something changed, while
        # still allowing pure column reordering under @preserve_columns.
