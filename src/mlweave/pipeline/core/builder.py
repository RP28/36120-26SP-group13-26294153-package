...
class PipelineStepBuilder:
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> MLWeavePipelineStep:
        ...
        if self.spec.mode == "stateful" and self.spec.fit_func is None:
            raise MLWeaveConfigurationError(
                f"'{self.spec.transform_func.__name__}' uses "
                "@stateful_pipeline_step but has no fit function. "
        ...
        return MLWeavePipelineStep(
            ...
            output_validators=tuple(
                sorted(self.spec.output_validators, key=lambda item: item.priority)
            ),
    ...
    def fit(self, func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        if self.spec.mode != "stateful":
            raise MLWeaveConfigurationError(
                f"'{self.spec.transform_func.__name__}' is not a stateful "
                "pipeline step. Use @stateful_pipeline_step before "
                "registering fit."
            )

        if self.spec.fit_func is not None:
            raise MLWeaveConfigurationError(
                "A fit function is already registered for "
                f"'{self.spec.transform_func.__name__}'."
            )
