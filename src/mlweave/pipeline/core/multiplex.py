...
def validate_multiplex(X: tuple[Any, ...], y: tuple[Any, ...] | None = None, *, require_multiple: bool = True) -> None:
    """Validate MLWeave's partition tuple contract.
    Partition 0 is always treated as training data. Remaining partitions are non-training datasets such as validation or test data.
    """
    ...
    if len(X) < minimum:
        raise MLWeaveValidationError(
            f"Multiplexed X must contain at least {minimum} partition(s). Partition 0 is the training dataset.")
    if any(part is None for part in X):
        raise MLWeaveValidationError("Multiplexed X cannot contain None partitions.")
    if y is None:
        return
    if not isinstance(y, tuple):
        raise MLWeaveValidationError("When multiplexed X is paired with y, y must also be a tuple.")
    if len(X) != len(y):
        raise MLWeaveValidationError(
            f"Multiplexed X and y must contain the same number of partitions: got {len(X)} X partition(s) and {len(y)} y partition(s).")
    for index, (X_part, y_part) in enumerate(zip(X, y)):
        ...
        if x_rows != y_rows:
            ...
            raise MLWeaveValidationError(f"Multiplexed {label} X/y row counts differ: {x_rows} != {y_rows}.")
...
def training_value(value: T | tuple[T, ...] | None) -> T | None:
    ...
    if isinstance(value, tuple):
        if not value:
            raise MLWeaveValidationError("A multiplexed tuple cannot be empty; partition 0 must be training data.")
        return value[0]
    return value
...
def partition_parameter(value: Any, index: int, count: int) -> Any:
    """Select aligned tuple metadata for one partition when unambiguous. Ordinary non-tuple values are shared across partitions. A tuple
    whose length exactly matches the dataset partition count is interpreted as partitioned metadata (for example per-split sample weights).
    """
    if isinstance(value, tuple) and len(value) == count:
        return value[index]
    return value

def partition_mapping(params: Mapping[str, Any] | None, index: int, count: int) -> dict[str, Any]:
    """Return one partition's view of runtime parameters without data copies."""
    if not params:
        return {}
    return {key: partition_parameter(value, index, count) for key, value in params.items()}
